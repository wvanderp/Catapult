...
This project is started to take all the unnecessary work out of uploading photos to wiki commons. This means that the uploader should do as little work as possible. forms should be filled automatically by automatic detection or extracting metadata from the photos. User interaction should be kept to a minimum.

On the other hand, the app should allow for maximum flexibility so that the user can create any template they want.

The UI of the app should thus both help the user and take work away from them, but also show the user what is is doing and what the software can do.
...
Documentation should be updated whenever code is changed. This includes inline code comments, README files, instructions, files in the docs folder, and any explanations in the app itself.
